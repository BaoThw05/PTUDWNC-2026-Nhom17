...
export function GoogleSignInButton({ callbackUrl }: { callbackUrl: string }) {
  ...
  return (
    <button
      ...
      className="flex items-center justify-center gap-2 rounded-full border border-black/15 px-4 py-2.5 font-medium transition hover:bg-black/[.04] disabled:opacity-60 dark:border-white/20 dark:hover:bg-white/[.06]"
    >
      ...
      {pending ? "Đang chuyển sang Google…" : "Tiếp tục với Google"}
    </button>
  );
}
