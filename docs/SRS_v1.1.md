> **SRS v1.1 (bản nháp đang cập nhật).** Tạo từ bản Markdown của SRS v1.0.0 ngày 17/09/2026. Mọi thay đổi so với v1.0.0 được liệt kê trong **Bảng Change Request** ngay sau Lịch sử thay đổi; phần thân tài liệu được sửa dần theo từng dòng CR. Bản gốc v1.0.0 vẫn giữ ở `SRS_Culinary_Blog_v1.0.0.md` để đối chiếu.

**GIÁO TRÌNH PHÁT TRIỂN ỨNG DỤNG WEB NÂNG CAO** · Phiên bản V4 · .NET 10 + Next.js App Router

# TÀI LIỆU ĐẶC TẢ YÊU CẦU PHẦN MỀM

_Software Requirements Specification (SRS)_ — _Tiêu chuẩn IEEE 830 / ISO/IEC/IEEE 29148:2018_

**Dự án: Blog Ẩm thực và Nấu ăn — _Culinary Blog_**

|Thuộc tính|Giá trị|
|---|---|
|**Phiên bản tài liệu**|1.1.0 (nháp)|
|**Ngày phát hành**|04/06/2026|
|**Trạng thái**|Đang cập nhật (Draft)|
|**Công nghệ Backend**|.NET 10 Minimal APIs, C#|
|**Công nghệ Frontend**|Next.js App Router, TypeScript|
|**Cơ sở dữ liệu**|PostgreSQL 16|
|**Object Storage**|Lưu trữ tương thích S3 (xem CR-14)|
|**Cache**|Redis 7|
...
|---|---|---|---|---|
|1.1.0|17/09/2026|Nhóm 17 (TV1 tổng hợp)|Sửa lỗi và làm rõ theo các quyết định S-01 → S-18; chi tiết ở Bảng Change Request.|Draft|
...
# BẢNG CHANGE REQUEST (SRS v1.1)

Mỗi dòng là một thay đổi so với SRS v1.0.0, lấy từ các quyết định trong `SRS_Culinary_Blog_v1.0.0_GiaiPhap.md` và ADR tương ứng trong `docs/adr/`. Khi đã sửa nội dung các mục bị ảnh hưởng trong tài liệu này, người phụ trách đổi trạng thái sang **Đã cập nhật**.

**Trạng thái:** ⬜ Đã chốt, chưa sửa nội dung · 🔄 Đang sửa · ✅ Đã cập nhật

| Mã CR | Quyết định | Nội dung thay đổi | Mục SRS bị ảnh hưởng | Phụ trách | ADR | Trạng thái |
|---|---|---|---|---|---|---|
| CR-01 | S-01 | Phát hành SRS v1.1 dạng Markdown trong repo, kèm bảng Change Request và ADR | Toàn tài liệu | TV1 | — | 🔄 |
| CR-02 | S-03 | Recipe xóa mềm vào thùng rác 30 ngày, có khôi phục; job định kỳ xóa thật và xóa ảnh; bảng con và Category xóa thật | FR-RCP-007, Chương 7 (Recipe) | TV2 | S-03 | ⬜ |
| CR-03 | S-04 | Kiểm soát đồng thời bằng cột hệ thống `xmin` (`uint Version`); client gửi `version`, lệch → 409 | FR-RCP-004, Chương 7 | TV2 | S-04 | ⬜ |
| CR-04 | S-05 | Xác thực theo mô hình BFF với Auth.js v5; đăng ký nhận `{ fullName, email, userName, password }` (khớp `SRS_..._GiaiPhap.md` bản 19/09) và đăng nhập luôn; Google: backend chỉ nhận `{ idToken }`, tự xác minh, tự sinh `userName` | FR-AUTH-001, FR-AUTH-003, 5.2, 5.3, 6.1 | TV1 | [ADR-001](./ADR/ADR-001-kien-truc-xac-thuc.md) | 🔄 |
| CR-05 | S-06 | Refresh token 32 byte, chỉ lưu SHA-256; thêm `FamilyId`, `RevokedReason`; bỏ `IsRevoked`; xoay vòng mỗi lần refresh, dùng lại trong 30 giây vẫn đổi được, quá 30 giây thu hồi cả family | FR-AUTH-001, FR-AUTH-004, NFR-SEC-002, 7.8 | TV1 | [ADR-002](./ADR/ADR-002-refresh-token.md) | 🔄 |
| CR-06 | S-05 | Đăng xuất `AllowAnonymous`, chỉ cần `refreshToken`, luôn trả 204; có access token thì chỉ thu hồi token của chính người đó | FR-AUTH-005 | TV1 | S-05 | ⬜ |
| CR-07 | S-05 | Đăng nhập: khóa 15 phút sau 5 lần sai → 423; `IsActive = false` → 403 `AUTH_ACCOUNT_DISABLED` (chỉ báo khi mật khẩu đúng); bỏ điều kiện "LockoutEnabled = false" | FR-AUTH-002 | TV1 | S-05 | ⬜ |
| CR-08 | S-17 | Hồ sơ: `PATCH /auth/me` chỉ đổi `displayName` (2–100 ký tự); `AvatarUrl` chỉ lấy từ Google; bỏ xác nhận email và đổi email/username bằng OTP khỏi v1 | FR-AUTH-006, FR-AUTH-007, 8.1 | TV1 | — | ⬜ |
...
| CR-10 | S-10 | Hợp đồng API: không vỏ bọc response; danh sách trả `PagedResult`; 422 cho validation; lỗi theo Problem Details (RFC 9457) có `code`, `traceId`; `sort=-field`; đổi tên thống nhất (`displayName` thay `fullName`…) | Chương 3, Chương 8, Phụ lục A, B | TV2 | S-10 | ⬜ |
...
| CR-15 | S-13 | Ghim MediatR 12.5.0 (MIT); `ApplicationUser` nằm ở Infrastructure, Domain không dùng NuGet; có architecture test | 2.5, 6.2, NFR-MAINT-004 | TV2 | S-13 | ⬜ |
| CR-16 | S-07 | Chiến lược cache: Output Cache + Redis, xóa theo tag | FR-CAT, NFR-PERF | TV4 | S-07 | ⬜ |
| CR-17 | S-08 | Tìm kiếm tiếng Việt không dấu bằng `pg_trgm` trên cột đã bỏ dấu | FR-SRCH-001 | TV4 | S-08 | ⬜ |
| CR-18 | S-14 | Sitemap sinh bằng Next.js; Hangfire recurring job cho các việc dọn dẹp (thùng rác, refresh token hết hạn, file mồ côi) | FR-JOB-003, 3.6 | TV4, TV3 | S-14 | ⬜ |
| CR-19 | S-16 | Viết lại NFR ở mức đo được trên máy demo; phần mở rộng chuyển sang "Định hướng" | Chương 4 | TV4 | S-16 | ⬜ |
...
- LỊCH SỬ THAY ĐỔI TÀI LIỆU — tr. 2
- MỤC LỤC — tr. 3
- CHƯƠNG 1. GIỚI THIỆU — tr. 6
  - 1.1. Mục đích Tài liệu — tr. 6
  - 1.2. Phạm vi Sản phẩm — tr. 6
    - 1.2.1. Tên và Định danh — tr. 6
    - 1.2.2. Mô tả Sản phẩm — tr. 6
    - 1.2.3. Những gì KHÔNG thuộc phạm vi — tr. 7
  - 1.3. Định nghĩa, Từ viết tắt và Ký hiệu — tr. 7
  - 1.4. Tài liệu Tham chiếu — tr. 8
  - 1.5. Tổng quan Tài liệu — tr. 10
- CHƯƠNG 2. MÔ TẢ TỔNG QUAN HỆ THỐNG — tr. 11
  - 2.1. Bối cảnh Sản phẩm — tr. 11
    - 2.1.1. Vị trí trong Hệ sinh thái — tr. 11
    - 2.1.2. Quan hệ với Hệ thống Ngoài — tr. 11
  - 2.2. Chức năng Sản phẩm Tổng quát — tr. 12
  - 2.3. Các Lớp Người dùng và Đặc điểm — tr. 12
  - 2.4. Môi trường Vận hành — tr. 13
    - 2.4.1. Môi trường Server (Production) — tr. 13
    - 2.4.2. Môi trường Phát triển (Development) — tr. 13
    - 2.4.3. Yêu cầu Trình duyệt Client — tr. 14
  - 2.5. Ràng buộc Thiết kế và Hiện thực — tr. 14
  - 2.6. Giả định và Phụ thuộc — tr. 15
    - 2.6.1. Giả định — tr. 15
    - 2.6.2. Phụ thuộc Bên ngoài — tr. 15
- CHƯƠNG 3. YÊU CẦU CHỨC NĂNG CHI TIẾT — tr. 17
  - 3.1. Module Xác thực và Quản lý Người dùng (FR-AUTH) — tr. 17
    - FR-AUTH-001: Đăng ký Tài khoản (User Registration) — tr. 17
    - FR-AUTH-002: Đăng nhập bằng Email/Mật khẩu (Local Login) — tr. 18
    - FR-AUTH-003: Đăng nhập bằng Google OAuth 2.0 — tr. 19
    - FR-AUTH-004: Làm mới Access Token (Token Refresh) — tr. 20
    - FR-AUTH-005: Đăng xuất (Logout / Token Revocation) — tr. 21
    - FR-AUTH-006: Xem Hồ sơ Cá nhân (View Profile) — tr. 22
    - FR-AUTH-007: Cập nhật Hồ sơ Cá nhân (Update Profile) — tr. 23
  - 3.2. Module Quản lý Danh mục (FR-CAT) — tr. 23
    - FR-CAT-001: Xem Danh sách Danh mục — tr. 23
    - FR-CAT-002: Xem Chi tiết Danh mục và Công thức — tr. 24
    - FR-CAT-003: Tạo Danh mục Mới [Admin] — tr. 25
    - FR-CAT-004: Cập nhật Danh mục [Admin] — tr. 26
    - FR-CAT-005: Xóa Danh mục [Admin] — tr. 26
  - 3.3. Module Quản lý Công thức Nấu ăn (FR-RCP) — tr. 27
    - FR-RCP-001: Xem Danh sách Công thức (Paginated + Filtered + Sorted) — tr. 27
    - FR-RCP-002: Xem Chi tiết Công thức — tr. 28
    - FR-RCP-003: Tạo Công thức Nấu ăn Mới [Author/Admin] — tr. 29
    - FR-RCP-004: Cập nhật Công thức [Author-Owner/Admin] — tr. 30
    - FR-RCP-005: Xuất bản / Hủy Xuất bản Công thức — tr. 31
    - FR-RCP-006: Lưu trữ Công thức (Archive) — tr. 32
    - FR-RCP-007: Xóa Công thức [Author-Owner/Admin] — tr. 32
    - FR-RCP-008: Quản lý Ảnh Công thức (Upload / Set Primary / Delete) — tr. 33
    - FR-RCP-009: Quản lý Nguyên liệu (CRUD RecipeIngredient) — tr. 34
    - FR-RCP-010: Quản lý Các bước Thực hiện (CRUD RecipeStep) — tr. 35
  - 3.4. Module Tìm kiếm và Phân trang (FR-SRCH) — tr. 36
    - FR-SRCH-001: Tìm kiếm Toàn văn bản (Full-Text Search) — tr. 36
    - FR-SRCH-002/003/004: Lọc, Sắp xếp và Phân trang (Tóm tắt) — tr. 37
  - 3.5. Module Quản lý Tệp tin (FR-FILE) — tr. 37
  - 3.6. Module Background Jobs (FR-JOB) — tr. 38
  - 3.7. Module Quan sát Hệ thống (FR-OBS) — tr. 39
- 4\. Yêu cầu Phi Chức năng (NFR) — tr. 40
  - 4.1. Hiệu năng (NFR-PERF) — tr. 40
  - 4.2. Bảo mật (NFR-SEC) — tr. 41
  - 4.3. Khả năng Sử dụng (NFR-USE) — tr. 42
  - 4.4. Độ tin cậy (NFR-REL) — tr. 42
  - 4.5. Khả năng Bảo trì (NFR-MAINT) — tr. 43
  - 4.6. Khả năng Mở rộng (NFR-SCALE) — tr. 44
  - 4.7. Tối ưu SEO (NFR-SEO) — tr. 44
- 5\. Yêu cầu Giao diện Ngoài — tr. 46
  - 5.1. Giao diện Người dùng (UI) — tr. 46
  - 5.2. Giao diện Phần mềm – REST API — tr. 47
  - 5.3. Giao diện Dịch vụ Bên thứ ba — tr. 47
  - 5.4. Giao diện Phần cứng — tr. 48
- 6\. Kiến trúc Hệ thống — tr. 50
  - 6.1. Tổng quan Kiến trúc — tr. 50
  - 6.2. Kiến trúc Backend – Clean Architecture — tr. 50
  - 6.3. CQRS + MediatR Pipeline — tr. 51
  - 6.4. Mô hình Quan hệ Thực thể (ERD tóm tắt) — tr. 52
  - 6.5. Triển khai – Docker Compose — tr. 52
- 7\. Mô hình Dữ liệu — tr. 54
  - 7.1. BaseEntity (Abstract) — tr. 54
  - 7.2. Recipe — tr. 54
    - 7.2.1. RecipeNutrition (Owned Entity — cột trong bảng Recipes) — tr. 56
  - 7.3. RecipeStep — tr. 57
  - 7.4. RecipeIngredient — tr. 57
  - 7.5. RecipeImage — tr. 58
  - 7.6. Category — tr. 58
  - 7.7. ApplicationUser (extends IdentityUser) — tr. 58
  - 7.8. RefreshToken — tr. 59
- 8\. Đặc tả REST API — tr. 61
  - 8.1. Authentication Module (/auth) — tr. 61
  - 8.2. Categories Module (/categories) — tr. 62
  - 8.3. Recipes Module (/recipes) — tr. 63
  - 8.4. Recipe Images (/recipes/{id}/images) — tr. 64
  - 8.5. Recipe Steps (/recipes/{id}/steps) — tr. 64
  - 8.6. Recipe Ingredients (/recipes/{id}/ingredients) — tr. 65
  - 8.7. Health Check Endpoints — tr. 65
- Phụ lục A – HTTP Status Codes — tr. 67
- Phụ lục B – Application Error Codes — tr. 67
- Phụ lục C – Từ điển Thuật ngữ — tr. 69
...
|Quản trị viên<br>(Admin)|Người quản lý hệ<br>thống với quyền<br>cao nhất. Được<br>gán thủ công qua<br>database seeding.|Có tài khoản &<br>role Admin|+ Tất cả quyền của Author.<br>+ Quản lý (CRUD) danh<br>mục. + Sửa/xóa bất kỳ<br>recipe của bất kỳ Author. +<br>Truy cập Hangfire<br>Dashboard. + Xem<br>structured logs.|Trung<br>bình (số<br>lượng<br>ít)|
...
|**HTTP Status Code trả**<br>**về**|200 OK – Thành công. 401 Unauthorized – Chưa đăng nhập. 404 Not Found – User không tồn tại.|
...
|**Luồng chính (Happy**<br>**Path)**|1. Author gửi PATCH /api/v1/recipes/{id}/publish (để xuất bản) hoặc<br>PATCH /api/v1/recipes/{id}/unpublish.<br>2. PublishRecipeCommand dispatch với isPublish = true/false.<br>3. Kiểm tra resource-based authorization.<br>4. Gọi domain method: recipe.Publish() hoặc recipe.Unpublish().<br>5. recipe.Publish() kiểm tra: Steps.Count == 0 → Throw<br>DomainException("Recipe phải có ít nhất 1 bước thực hiện.").<br>6. Set Status = Published/Draft, UpdatedAt = DateTime.UtcNow.<br>7. SaveChangesAsync(), invalidate cache.<br>8. Trả về HTTP 200 OK với RecipeDto.|
...
|**HTTP Method &**<br>**Endpoint**|PATCH  /api/v1/recipes/{id:guid}/publish  \|  PATCH  /api/v1/recipes/{id:guid}/unpublish|
...
|**NFR-SEC-003 Rate**<br>**Limiting**|Giới hạn yêu cầu theo IP để ngăn brute force và DDoS:<br>• Auth<br>endpoints (/auth/*): 10 request/phút/IP.<br>• API chung: 100<br>request/phút/IP.<br>• Upload endpoints: 5 request/phút/IP.<br>Implementation: ASP.NET Core Rate Limiting middleware<br>(Fixed Window, sliding window cho auth). HTTP 429 khi vượt<br>giới hạn với Retry-After header.|
...
|Mã NFR|Yêu cầu chi tiết|
|---|---|
|**NFR-USE-001**<br>**Responsive Design**|Giao diện hiển thị chính xác trên tất cả breakpoints:<br>• Mobile:<br>320px – 767px (single column, touch-friendly).<br>• Tablet: 768px<br>– 1199px (2-column grid).<br>• Desktop: ≥ 1200px (full layout).<br>Framework: Tailwind CSS utility-first. Không sử dụng CSS<br>framework override. Kiểm thử: Chrome DevTools responsive<br>mode + BrowserStack (iOS, Android).|
|**NFR-USE-002**<br>**Accessibility (a11y)**|Tuân thủ WCAG 2.1 Level AA:<br>• Semantic HTML5: <article>,<br><nav>, <main>, <aside>.<br>• ARIA attributes: aria-label, aria-<br>expanded, role trên interactive elements.<br>• Keyboard<br>navigation: tất cả chức năng dùng được bằng bàn phím (Tab,<br>Enter, Escape).<br>• Color contrast ratio ≥ 4.5:1 (text) và ≥ 3:1 (UI<br>components).<br>• Screen reader: test với NVDA (Windows) và<br>VoiceOver (macOS/iOS).|
|**NFR-USE-003 Error**<br>**Messages**|Thông báo lỗi phải rõ ràng và actionable:<br>• API: trả về RFC<br>7807 Problem Details (type, title, status, detail, errors{}). •<br>Frontend: hiển thị ngay bên cạnh field lỗi (React Hook Form<br>inline validation).<br>• Server errors (5xx): hiển thị thông báo thân<br>thiện, không lộ stack trace.<br>• I18n-ready: error messages sử<br>dụng error code (không hardcode tiếng Việt/Anh).|
|**NFR-USE-004**<br>**Loading States**|Mọi async operation phải có visual feedback:<br>• Loading<br>skeleton: hiển thị trong khi fetch data (không blank screen). •<br>Optimistic update: UI cập nhật ngay, rollback nếu API fail. •<br>Toast notification: xác nhận thành công/thất bại sau write<br>operation.<br>• Progress indicator: upload ảnh hiển thị progress<br>bar (%) realtime.|
...
|/dashboard/recipes/new|Form tạo recipe mới<br>(multi-step wizard)|CSR|Bắt buộc<br>(Author/Admin)|
...
|POST|/auth/refresh|Làm mới<br>Access Token|Không (dùng<br>refreshToken)|{ refreshToken<br>}|200: {<br>accessToken,<br>refreshToken,<br>expiresIn }<br>401: token<br>hết hạn / bị<br>revoke|
...
|Clean Architecture|CA|Kiến trúc phần mềm của Robert C. Martin tách biệt<br>concerns theo layers (Domain, Application,<br>Infrastructure, Presentation). Dependency chỉ đi vào<br>trong (hướng Domain).|
