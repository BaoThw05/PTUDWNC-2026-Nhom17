...
| CR-04 | S-05 | Xác thực theo mô hình BFF với Auth.js v5; đăng ký nhận `{ email, password, displayName }` (không cần `userName`, `UserName` = email) và đăng nhập luôn; Google: backend chỉ nhận `{ idToken }` và tự xác minh | FR-AUTH-001, FR-AUTH-003, 5.2, 5.3, 6.1 | TV1 | [S-05](./adr/0001-S05-kien-truc-xac-thuc.md) | ⬜ |
| CR-05 | S-06 | Refresh token 32 byte, chỉ lưu SHA-256; thêm `FamilyId`, `RevokedReason`; bỏ `IsRevoked`; xoay vòng mỗi lần refresh, dùng lại trong 30 giây vẫn đổi được, quá 30 giây thu hồi cả family | FR-AUTH-001, FR-AUTH-004, NFR-SEC-002, 7.8 | TV1 | [S-06](./adr/0002-S06-refresh-token.md) | ⬜ |
