# 0001 · S-05 · Kiến trúc xác thực Next.js ↔ .NET
...
- **Đăng ký:** gọi `POST /api/v1/auth/register { email, password, displayName }` rồi `signIn("credentials")`. Không bắt buộc `userName` (Identity `UserName` = email).
...
- Rủi ro chấp nhận: access token xuống trình duyệt; phản hồi 409 (email trùng) và 423 cho phép dò tài khoản — giảm bằng rate limit 5 lần/phút (S-15).
